...
func GenerateGRPCTransport(service *Service) string {
	...
	defer file.Close()
	file.WriteString(transportHeading)
	// Generate adapters
...
const transportHeading string = `package transport
...
const templateG string = `func (g grpcTransport) %s(ctx context.Context, request *pb.%sRequest) (*pb.%sResponse, error) {
...
const templateDec string = `func decode%s(ctx context.Context, grpcRequest interface{}) (interface{}, error) {
...
const templateEnc string = `func encode%s(ctx context.Context, grpcResponse interface{}) (interface{}, error) {
...
const encFieldTypeTemplate = `		%s: resp.%s,
`
